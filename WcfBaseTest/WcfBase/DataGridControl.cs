...
namespace WcfBase.Controls
{
    ...
    [Serializable]
    public abstract class DataGridOption : IDataGridOption
    {
        public string pluginName = "";
        public string tagName = "";

        protected Dictionary<string, string> SetProperties = new Dictionary<string, string>();
    ...
    public class ColumnOptionsList : List<ColumnOptions> { }

    public class ColumnDefList : List<ColumnDef> { }
    ...
    public class DataTableNetDataGridOption : DataGridOption
    {
        ...
        private ColumnOptionsList _columns;
        [PersistenceMode(PersistenceMode.InnerProperty)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Content)]
        public ColumnOptionsList Columns
        {
            get
            {
                if (_columns == null)
                    _columns = new ColumnOptionsList();
                return _columns;
            }
            set
            {
                _columns = value;
                SetProperties["aoColumns"] = GetJsonValue(_columns);
            }
        }

        private ColumnDefList _columnDefs;
        [PersistenceMode(PersistenceMode.InnerProperty)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Content)]
        public ColumnDefList ColumnDefs
        {
            get
            {
                if (_columnDefs == null)
                    _columnDefs = new ColumnDefList();
                return _columnDefs;
            }
            set
            {
                _columnDefs = value;
                SetProperties["aoColumnDefs"] = GetJsonValue(_columnDefs);
            }
        }

        

        private string GetJsonValue(List<ColumnOptions> colOptions)
        {
            var scriptString = "";
            colOptions.ForEach(c => scriptString += c.GetScript("") + ",");
            scriptString = scriptString.Remove(scriptString.LastIndexOf(","));
            return "[" + scriptString + "]";
        }

        private string GetJsonValue(List<ColumnDef> colOptions)
        {
            var scriptString = "";
            colOptions.ForEach(c => scriptString += c.GetScript("") + ",");
            scriptString = scriptString.Remove(scriptString.LastIndexOf(","));
    ...
    [ParseChildren(
        typeof(DataTableNetDataGridOption),
           DefaultProperty = "DataGridOptions",
           ChildrenAsProperties = true
           )]
    [PersistChildren(false)]
    [ToolboxData("<{0}:DataGridControl runat=server></{0}:DataGridControl>")]
    public class DataTableNetDataGrid : PlaceHolder
    {
        [PersistenceMode(PersistenceMode.InnerDefaultProperty)]
        [Browsable(true)]
        [DesignerSerializationVisibility(DesignerSerializationVisibility.Content)]
        [TypeConverter(typeof(ExpandableObjectConverter))]
        public DataTableNetDataGridOption DataGridOptions { get; set; }
        ...
        protected override void Render(HtmlTextWriter writer)
        {
            ...
            {
                var dataGrid = new HtmlGenericControl(DataGridOptions.tagName);
                dataGrid.ID = this.ID;
                this.Controls.Add(dataGrid);

                this.Controls.Add(new LiteralControl(DataGridOptions.GetScript(dataGrid.ClientID)));
            }
